...
void ls(int depth, DIR* d, char path[300]){
 struct dirent * dp; //pointer to dirent struct
 struct stat st;
 //parameter the directory name (with path) we want to open
 ...
 while (dp=readdir(d)){ 
   ...
   printf("%s\n",dp->d_name);
   if(S_ISDIR(st.st_mode) && strcmp(".", dp->d_name) && strcmp("..", dp->d_name)) {
    char next[300];
    //printf("here: %s\n", path);
    strcpy(next, path);
    strcat(next, "/");
    strcat(next, dp->d_name);
    ls(depth+1, opendir(next), next);
   }
 }
