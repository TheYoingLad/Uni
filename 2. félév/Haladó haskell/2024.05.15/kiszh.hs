...
keywords :: [String]
keywords = ["true", "false", "not", "sign", "if", "then", "do", "for", "lam", "end", "create", "delete"]
...
data Statement
  ...
  | Create String             -- create v
  | Delete String             -- delete v
  deriving Show
...
statement = asum [sIf, sWhile, sAssign, sCreate, sDelete]
...
sCreate = Create <$> (pKeyword "create" *> pNonKeyword)

sDelete :: Parser Statement
sDelete = Delete <$> (pKeyword "delete" *> pNonKeyword)
...
data InterpreterError
  ...
  | VariableExists { message :: String } -- 0-val való osztás hibaüzenettel
  deriving Show
...
evalStatement st = case st of
  ...
  Create s -> do
    ...
    case lookup s env of
      (Just _) -> throwError (VariableExists $ "Variable already exists\nSTACK TRACE:\n" ++ stackTrace)
      _ -> put $ updateEnv env s VNull
  Delete s -> do
    env <- get
    case lookup s env of
      (Just _) -> put $ deleteFromEnv env s
      _ -> pure ()
