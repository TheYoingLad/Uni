...
-- (c*)i?(c+)a{3}
p3 :: Parser ()
p3 = many (char 'c') >> optional (char 'i') >> some (char 'c') >> replicateM 3 (char 'a') >> pure ()

-- (alma|banana)?
p4 :: Parser ()
p4 = optional (string "alma" <|> string "banana") >> pure ()

-- [A-Z]{10}
p5 :: Parser ()
p5 = replicateM' 10 (satisfy (\x -> x >= 'A' && x <= 'Z')) >> pure ()

-- \d{2}
p6 :: Parser ()
p6 = replicateM' 2 (digitToInt <$> satisfy isDigit) >> pure ()
...
-- (ab)?ba+.*
p9 :: Parser ()
p9 = optional (string "ab") >> char 'b' >> some (char 'a') >> many anychar >> pure ()

-- \d{2,}-?$
p10 :: Parser ()
p10 = many (replicateM 2 (digitToInt <$> satisfy isDigit)) >> optional (char '-') >> eof

-- [A-Z]*[1,2,3,9]?(A|Z)$
p11 :: Parser ()
p11 = many (satisfy (\x -> x >= 'A' && x <= 'Z')) >> optional (satisfy (\x -> elem x ['1','2','3','9'])) >> (char 'A' <|> char 'Z') >> eof

-- a+b*c?d{4}e{5,}
p12 :: Parser ()
p12 = some (char 'a') >> many (char 'b') >> optional (char 'c') >> replicateM' 4 (char 'd') >> many (replicateM' 5 (char 'e')) >> pure ()

-- alm(a|ák)|banán(ok)?
p13 :: Parser ()
p13 = (string "alm" >> char 'a' <|> string "ák") <|> (string "banán" >> optional (string "ok") >> pure ())