...
instance Functor RoseTree where
  ...
  fmap f (RoseNode as) = RoseNode [fmap f a | a <- as]

instance Functor Tree2 where
  fmap :: (a -> b) -> Tree2 a -> Tree2 b
  fmap f (Leaf2 a) = Leaf2 (f a)
  fmap f (Node2 as bs) = Node2 (fmap f as) (fmap f bs)
...
instance Functor CrazyType where
  ...
  fmap f (C2 a const) = C2 (f a) const
  fmap f (C3 as) = C3 (fmap f as)

instance Functor (Either3 fixed1 fixed2) where
  fmap :: (a -> b) -> Either3 fixed1 fixed2 a -> Either3 fixed1 fixed2 b
  fmap f (Left3 fixed) = Left3 fixed
  fmap f (Middle3 fixed) = Middle3 fixed
  fmap f (Right3 a) = Right3 (f a)

instance Functor (Triplet fixed1 fixed2) where
  fmap :: (a -> b) -> Triplet fixed1 fixed2 a -> Triplet fixed1 fixed2 b
  fmap f (Triplet fixed1 fixed2 a) = Triplet fixed1 fixed2 (f a)

instance Functor (SplitTree fixed) where
  fmap :: (a -> b) -> SplitTree fixed a -> SplitTree fixed b
  fmap f (SplitTree fixeds fixed a as) = SplitTree fixeds fixed (f a) (fmap f as)

instance (Functor f, Functor g, Functor h) => Functor (TriCompose f g h) where
  fmap :: (a -> b) -> TriCompose f g h a -> TriCompose f g h b
  fmap f (TriCompose as) = TriCompose (fmap (fmap (fmap f)) as)
...
instance (Functor f) => Functor (Fix f) where
  ...
  fmap f (Fix as) = Fix (fmap (fmap f) as)

instance Functor (Join f) where
  fmap :: (a -> b) -> Join f a -> Join f b
  fmap f (Join as) = Join ((fmap f) .  as)

instance Functor (CrazyType2 fixed) where
   fmap :: (a -> b) -> CrazyType2 fixed a -> CrazyType2 fixed b
   fmap f (SingleA fixed) = SingleA fixed
   fmap f (SingleB a) = SingleB (f a)
   fmap f (Translate fgv) = Translate (f . fgv)
