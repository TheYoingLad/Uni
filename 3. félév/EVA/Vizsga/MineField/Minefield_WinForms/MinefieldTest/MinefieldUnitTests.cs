...
namespace Minefield.Test
{
    ...
    public class MinefieldUnitTests : IDisposable
    {
        ...
        public void Init()
        {
            ...
            _mockFileManager.Setup(fm => fm.LoadFile(It.IsAny<String>())).Returns(() => (_testTable, testPlayer, new TimeSpan(0,0,0,4,500)));
        ...
        [TestMethod]
        public void MinefieldMinesDescendingTest()
        {
            _testModel.LoadGame("");
            _testModel.TogglePaused();
            _mockTimer.Raise(t => t.Elapsed += null, (EventArgs.Empty as ElapsedEventArgs)!);
        ...
        [TestMethod]
        public void MinefieldGameOverMovementTest()
        {
        ...
        [TestMethod]
        public void MinefieldGameOverMineDescendingOnPlayerTest()
        {
            bool gameOverEventRaised = false;
            _testModel.GameOver += (Object? _, MinefieldEventArgs _) => { gameOverEventRaised = true; };
            _testModel.LoadGame("");
        ...
        [TestMethod]
        public void MinefieldSaveGameCalledTest()
        {
            _testModel.StartNewGame();
            _testModel.SaveGame("");
            _mockFileManager.Verify(fm => fm.SaveFile("", It.IsAny<TimeSpan>(), It.IsAny<Entity[,]>()), Times.Once());
        }

        [TestMethod]
        public void MinefieldLoadGameCalledTest()
        {
            _testModel.StartNewGame();
            _testModel.LoadGame("");
            _mockFileManager.Verify(fm => fm.LoadFile(""), Times.Once());
        }
        [TestMethod]
        [ExpectedException(typeof(IOException))]
        public void MinefieldSaveGameFailedTest()
        {
            _mockFileManager.Setup(fm => fm.SaveFile(It.IsAny<String>(), It.IsAny<TimeSpan>(), It.IsAny<Entity[,]>())).Callback(() => throw new IOException());
            _testModel.StartNewGame();
            _testModel.SaveGame("");
        }
        ...
        [TestMethod]
        [ExpectedException(typeof(IOException))]
        public void MinefieldLoadGameFailedTest()
        {
            _mockFileManager.Setup(fm => fm.LoadFile(It.IsAny<String>())).Returns(() => throw new IOException());
            _testModel.LoadGame("");
        }

        [TestMethod]
        public void MinefieldLoadGameTest()
        {
            _testModel.LoadGame("");
