...
namespace LaserPigs.Persistence
{
    ...
    {
        public async Task<(Player p1, Player p2, int instructionIndex)> LoadAsync(Stream stream)
        {
            Player p1;
            Player p2;
            string[] data;
        ...
        public async Task SaveAsync(Stream stream, Player p1, Player p2, int instructionIndex)
        {
            try
            {
                ...
                {
                    foreach (string s in data) await sr.WriteLineAsync(s);
                }
            }
            catch (Exception e)
            {
        ...
        {
            await SaveAsync(File.OpenWrite(path), p1, p2, instructionIndex);
        }
